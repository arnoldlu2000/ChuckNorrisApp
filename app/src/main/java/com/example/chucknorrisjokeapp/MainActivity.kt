...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var newList : ArrayList<Joke>
    //private lateinit var imageId : Array<Int>
    private lateinit var joke : Array<String>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        newList = arrayListOf<Joke>()
        fillingJokes()
    }

    private fun fillingJokes(){

        //for (i in imageId.indices){
        for (i in joke.indices){
            //val items = Items(imageId[i], joke[i])
                val items = Joke(emptyArray(), "", "", "", "", "", joke[i])
            newList.add(items)
        }

        newRv.adapter = JokeAdapter(newList)
